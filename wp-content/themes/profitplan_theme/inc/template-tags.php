...
if ( ! function_exists( 'twenty_twenty_one_posted_on' ) ) {
	...
	function twenty_twenty_one_posted_on() {
		...
		printf(
			...
			esc_html__( 'Published %s', 'profitplan_theme' ),
	...
	function twenty_twenty_one_posted_by() {
		if ( ! get_the_author_meta( 'description' ) && post_type_supports( get_post_type(), 'author' ) ) {
			...
			printf(
				...
				esc_html__( 'By %s', 'profitplan_theme' ),
...
if ( ! function_exists( 'twenty_twenty_one_entry_meta_footer' ) ) {
	...
	function twenty_twenty_one_entry_meta_footer() {
		...
		if ( ! is_single() ) {
			...
			if ( is_sticky() ) {
				echo '<p>' . esc_html_x( 'Featured post', 'Label for sticky posts', 'profitplan_theme' ) . '</p>';
			}
			...
			if ( has_category() || has_tag() ) {
				...
				if ( $categories_list ) {
					printf(
						...
						'<span class="cat-links">' . esc_html__( 'Categorized as %s', 'profitplan_theme' ) . ' </span>',
				...
				if ( $tags_list ) {
					printf(
						...
						'<span class="tags-links">' . esc_html__( 'Tagged %s', 'profitplan_theme' ) . '</span>',
...
if ( ! function_exists( 'twenty_twenty_one_the_posts_navigation' ) ) {
	...
	function twenty_twenty_one_the_posts_navigation() {
		the_posts_pagination(
			array(
				'before_page_number' => esc_html__( 'Page', 'profitplan_theme' ) . ' ',
				'mid_size'           => 0,
				'prev_text'          => sprintf(
					'%s <span class="nav-prev-text">%s</span>',
					is_rtl() ? twenty_twenty_one_get_icon_svg( 'ui', 'arrow_right' ) : twenty_twenty_one_get_icon_svg( 'ui', 'arrow_left' ),
					wp_kses(
						__( 'Newer <span class="nav-short">posts</span>', 'profitplan_theme' ),
						array(
							'span' => array(
								'class' => array(),
							),
				...
				'next_text'          => sprintf(
					...
					wp_kses(
						__( 'Older <span class="nav-short">posts</span>', 'profitplan_theme' ),
						array(
							'span' => array(
								'class' => array(),
							),
